# -*- coding: utf-8 -*-
import logging, aiohttp, asyncio, ssl, uuid
from typing import List, Dict, Any, Optional
from .retrieval import RetrievalResult
from config import (
    LLM_PROVIDER, GIGACHAT_CREDENTIALS, GIGACHAT_MODEL, GIGACHAT_TEMPERATURE, 
    GIGACHAT_MAX_TOKENS, GIGACHAT_TIMEOUT, GIGACHAT_VERIFY_SSL, 
...
class LLMClient:
    ...
    async def _generate_gigachat_async(self, prompt, sys_prompt, max_tokens, temp) -> str:
        ...
        # ИСПРАВЛЕНО: Генерируем валидный UUID и добавляем Accept
        auth_h = {
        ...
        try:
            async with session.post(
                ...
                if r.status != 200: 
                    text = await r.text()
                    return f"Ошибка авторизации Сбера ({r.status}): {text}"
            ...
            async with session.post(
                ...
                if r.status != 200: 
                    text = await r.text()
                    return f"Ошибка API Сбера ({r.status}): {text}"
    ...
    async def _generate_local_async(self, prompt, sys_prompt, max_tokens, temp) -> str:
        try:
            ...
            async with session.post(
                ...
                if r.status != 200: 
                    return f"Ошибка Local LLM: {r.status}"
                return (await r.json()).get("response", "").strip()
        except Exception as e: 
            return f"Ошибка Local: {e}"
